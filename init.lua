...
local function head_particle(player, texture)
	...
	pos.y = pos.y + (prop.eye_height or 1.23) -- mouth level
...
if damage_enabled and minetest.settings:get_bool("enable_stamina") ~= false then
	...
	minetest.register_on_respawnplayer(function(player)
		...
		if monoids then
			if (stamina.players[name].sprint ~= nil) then
				player_monoids.speed:del_change(player,
					stamina.players[name].sprint)
			end
			if (stamina.players[name].sprint ~= nil) then
				player_monoids.jump:del_change(player,
					stamina.players[name].jump)
			end
...
if minetest.get_modpath("hudbars") then
	...
	hb.register_hudbar("stamina", 0xFFFFFF, S("Hunger"), { bar = "hudbars_bar_stamina.png" }, 20, 20, false)
else
